...
import { Clock, CheckCircle2, Flame, Link as LinkIcon, BookOpen, Layers, RotateCcw, AlertTriangle, Plus, Quote } from 'lucide-react';
...
export default function Home() {
  const { profile, updateProfile, tasks, chains, studyBlocks, documents, resetFocusTime, addChain, wisdomQuotes } = useGlobalStore();
  const [showResetModal, setShowResetModal] = React.useState(false);
  const [isAddingChain, setIsAddingChain] = React.useState(false);
  const [newChainTitle, setNewChainTitle] = React.useState('');
  const [randomQuote, setRandomQuote] = React.useState<any>(null);

  React.useEffect(() => {
    if (wisdomQuotes && wisdomQuotes.length > 0) {
      const randomIndex = Math.floor(Math.random() * wisdomQuotes.length);
      setRandomQuote(wisdomQuotes[randomIndex]);
    }
  }, [wisdomQuotes]);
  ...
  return (
    ...
    <div className="flex flex-col gap-7 h-full pb-10">
      ...
      {/* Wisdom Quote */}
      <GlassCard delay={0.1} className="px-5 py-4 relative overflow-hidden group">
        <div className="absolute -top-6 -right-6 opacity-5 pointer-events-none transition-transform group-hover:scale-110 group-hover:rotate-12 duration-700">
          <Quote size={100} />
        </div>
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-1.5 text-primary/80">
            <Quote size={14} />
            <span className="text-[10px] font-bold tracking-[0.15em] uppercase">Daily Wisdom</span>
          </div>
          {randomQuote ? (
            <div>
              <p className="text-base md:text-lg font-medium leading-relaxed italic text-foreground/90 mb-1.5">"{randomQuote.quote}"</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground font-semibold">
                <span>— {randomQuote.author || 'Unknown'}</span>
                {randomQuote.source && <span className="opacity-50">({randomQuote.source})</span>}
              </div>
            </div>
          ) : (
            <div>
              <p className="text-base md:text-lg font-medium leading-relaxed italic text-muted-foreground/50 mb-1.5">"A journey of a thousand miles begins with a single step."</p>
              <div className="flex items-center gap-2 text-xs text-muted-foreground/40 font-semibold">
                <span>— Lao Tzu</span>
              </div>
            </div>
          )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-2">
        {[
          ...
          <GlassCard key={i} delay={0.4 + (i * 0.06)} className="p-5 group">
