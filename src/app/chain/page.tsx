          ...
          <div className="relative">
            ...
            {showSelector && (
              <div className="absolute left-0 top-12 z-20 bg-card border border-border/50 rounded-xl shadow-xl py-1.5 min-w-[220px]">
                {chains.map(c => (
                  <div key={c.id} onClick={() => { setActiveChainId(c.id); setShowSelector(false); }}
                    className={`flex items-center justify-between w-full px-4 py-2.5 text-sm font-medium hover:bg-muted/30 transition-colors cursor-pointer ${c.id === activeChainId ? 'text-primary' : ''}`}>
                ...
                return (
                  <React.Fragment key={dayStr}>
                    ...
                    <button
                      ...
                      className={`aspect-square rounded-[6px] flex items-center justify-center text-[10px] font-bold transition-all relative hover:scale-125 hover:z-10 active:scale-90 ${entry
